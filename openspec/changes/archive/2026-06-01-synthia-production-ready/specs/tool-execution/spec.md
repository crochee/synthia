...
Each tool category SHALL have a default timeout. Tool execution SHALL be wrapped in tokio::time::timeout with the configured value. When timeout is exceeded, the tool call SHALL be cancelled and return a timeout error.
...
#### Scenario: File read completes within timeout
- **WHEN** a file read operation completes within 5 seconds
