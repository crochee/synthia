...
Define the single canonical `LoopDetectorSet` in `synthia-guardian` that combines five independent detectors (DoomLoop, GenericRepeat, PingPong, PollNoProgress, GlobalCircuit) and exposes a tuple-return API `(LoopStatus, Option<LoopAction>)`. This spec was created by archiving the `loop-detector-unification` change.
