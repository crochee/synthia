...
The `synthia_agent::tools::agent_tools` module must be decomposed from a single 1300+ line file into 7 focused sub-files (each < 300 lines) while preserving the existing public API surface (`agent_tools::*`) through a shim re-export. This is a pure file-level refactor with zero behavior change, verified by all existing tests passing.
