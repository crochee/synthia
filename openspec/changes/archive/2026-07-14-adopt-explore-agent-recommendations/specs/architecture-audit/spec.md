...
The `architecture-audit` spec SHALL include a `## Purpose` section that summarizes the audit's scope in 1-2 sentences.

#### Scenario: Purpose section present
- **WHEN** reading `openspec/specs/architecture-audit/spec.md`
- **THEN** the file SHALL contain a `## Purpose` section preceding `## Requirements` with a non-empty, non-TBD description
