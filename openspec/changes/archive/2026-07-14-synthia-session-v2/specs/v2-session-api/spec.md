...
_None._ The v3 rollout is additive: legacy `synthia-session` types remain reachable via `#[deprecated]` aliases in `crates/synthia-session/src/deprecation.rs` (commit `facd3a9`), and the legacy `POST /run_stream` server route remains reachable via `crates/synthia-server/src/routes/legacy_run_stream.rs` (commit `07e657e`) for one minor version. The existing `v2-session-api` `SessionSummary.parent_id` and `SessionManager::list_sessions_for_user(parent_id)` filter behavior documented in `openspec/specs/v2-session-api/spec.md` is preserved as-is via the re-export shim and is captured in the Archive Note at the top of [tasks.md](../tasks.md).