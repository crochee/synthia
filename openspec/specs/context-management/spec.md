...
Replace abrupt section-dropping in context pruning with three-stage progressive degradation (`Soft Trim` → `Hard Clear` → `分级压缩`) plus safety thresholds (16K/32K) and KV Cache prefix tracking. This ensures context utilization can grow without sudden large information loss and preserves prompt-cache hit rate under pressure.
