...
Agent runtime 6 个关键路径边界的 span 创建（session / turn / llm.call / tool.execute / compaction / guardian.check），全部置于 `#[cfg(feature = "otel")]` 守卫下，无 feature 时退化为 no-op。这落实 P9 可观测性原则——无法测量的无法优化——同时不修改 prompt 前缀（P1 前缀一致性）。
