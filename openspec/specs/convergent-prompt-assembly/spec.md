...
## Purpose
Converges all prompt-assembly paths in the workspace on `synthia_context::assembler::ContextAssembler`
as the single public entry point. Removes the private `ContextBuilder` struct from `synthia-agent`
and any system-prompt composition outside the assembler. Exposes a `section_by_name` lookup for
post-assembly inspection by `StreamBuilder` self-reflection.
## Requirements
