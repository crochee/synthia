# tool-output-offloading Specification

## Purpose

When a tool returns output that exceeds configurable byte or line thresholds, the system SHALL persist the full output to a deterministic file path and return only a head/tail summary to the LLM context. This reduces context pressure, protects KV Cache prefix stability, and keeps large outputs available to the model via the existing `read` tool.
