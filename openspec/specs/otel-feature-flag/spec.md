...
cargo feature `otel`（默认禁用）控制 OTel 依赖引入与编译路径；无 feature 时 `synthia-telemetry` 退化为纯 `tracing` console 输出。这满足 P3 按需加载原则，使 SDK 场景（个人 SDK / IDE 内嵌）不必背负 OTel 体积与编译开销。
