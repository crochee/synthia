...
/// `GET /health` is the liveness probe used by orchestrators and
/// must always return 200 with `{ status, version }`. Pin this
/// against the full router so we exercise the public route
/// surface end-to-end (including the public/protected split and
/// CORS layer).
#[tokio::test]
async fn health_endpoint_returns_200_with_status_and_version() {
...
/// `GET /health` is mounted under the public router — it MUST NOT
/// require the `Authorization` header. A previous regression moved
/// it under `protected` and broke orchestrator probes.
#[tokio::test]
async fn health_endpoint_does_not_require_authorization() {
    let app = make_app().await;
    let req = Request::builder()
        .uri("/health")
