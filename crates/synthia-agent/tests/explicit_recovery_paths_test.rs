...
//! emits `AgentEvent::System(SystemEvent::Recovery { .. })` for
//! observability. Each test drives the full `StreamBuilder`
//! end-to-end with a `FakeProvider` + `FakeTool` (see `test_support`).
...
use synthia_agent::{
    ...
    events::{SessionEndReason, SystemEvent},
...
fn count_recovery(events: &[AgentEvent]) -> usize {
    events
        .iter()
        .filter(|e| {
            matches!(e, AgentEvent::System(SystemEvent::Recovery { .. }))
        })
        .count()
}

/// L1 (truncate) path: a tool that returns 50KB of output must trigger
/// the agent loop's L1 truncation hook and emit a `Recovery` event
/// with `level_number == 1`.
#[tokio::test]
async fn l1_truncate_emits_recovery_applied_for_oversized_tool_output() {
    ...
    let recovery_count = count_recovery(&events);
    assert!(
        recovery_count >= 1,
        "expected at least one Recovery event for oversized tool output, got events: {:#?}",
        events
            .iter()
            .map(|e| match e {
                AgentEvent::Model(ContentPart::ToolResult(tr)) => {
                    let first = tr
                        ...
                        .unwrap_or_default();
                    format!(
                        "ToolCallCompleted(id={}, is_error={:?}, first={:?})",
                        tr.tool_use_id, tr.is_error, first
                    )
                }
                AgentEvent::System(SystemEvent::Recovery {
                    ...
                    tool_name,
                    ..
                }) => format!("Recovery(L{level_number}, {tool_name:?})"),
                other => format!("{:?}", other),
            })
            .collect::<Vec<_>>()
    );
    ...
    let l1 = events
        .iter()
        .find_map(|e| {
            if let AgentEvent::System(SystemEvent::Recovery {
                level_number,
                tool_name,
                message,
                iteration,
        ...
        .expect("expected level_number=1 Recovery event");
    let (level_number, tool_name, message, iteration) = l1;
    ...
    assert!(iteration.unwrap_or(0) > 0, "iteration must be > 0");
    ...
    let completed = events
        .iter()
        .find_map(|e| {
            if let AgentEvent::Model(ContentPart::ToolResult(tr)) = e {
                if tr.tool_use_id == "call-1" && tr.is_error == Some(false) {
                    let first = tr
                        .content
                        .iter()
                        .find_map(|p| {
...
/// L1 truncate must be a no-op for small outputs: NO `Recovery`
...
async fn l1_truncate_does_not_emit_recovery_applied_for_small_output() {
    ...
    assert!(
        ...
        "small tool output should NOT trigger Recovery, got: {:#?}",
        recovery_for_tool
    );
...
/// `run_recovery_cascade` and emit a `Recovery` event with the
...
/// recover from must:
///
///  1. Yield a `Recovery` event with `tool_name == Some("llm_sample")`
...
async fn llm_sampling_error_runs_recovery_cascade_and_continues() {
    ...
    // Find the Recovery event. The level is 5 (L5 Reset) because no
    // `web_fetch` / registered fallback exists for `"llm_sample"` and
    // `context_token_budget` ratio stays low.
    let recovery = events
        .iter()
        .find_map(|e| {
            if let AgentEvent::System(SystemEvent::Recovery {
        ...
        .unwrap_or_else(|| {
            panic!(
                "expected a Recovery event after LLM sampling error, got: {:#?}",
                events
                    .iter()
                    .map(|e| match e {
                        ...
                        AgentEvent::System(SystemEvent::Recovery {
                            level_number, ..
                        }) => format!("Recovery(L{level_number})"),
...
/// Task 4 (companion): When the LLM fails persistently, the cascade
/// fires L5 Reset repeatedly. Without a registered fallback or cooldown
/// between resets, the cascade resets on every failure and the agent
/// loop continues.
///
/// NOTE: `StepSample` calls `complete_with_stream` first and, if the
/// stream ends without `IsDone`, falls back to synchronous `complete()`.
/// The FakeProvider shares a single `call_count` between the two, so
/// each "logical" LLM call consumes TWO `completion_errors` slots.
/// The test scripts 4 errors (idx 0-3) for 2 LLM errors + 1 success
/// (idx 4).
#[tokio::test]
async fn llm_sampling_cascade_emits_recovery_applied_with_llm_sample_tool_name()
{
    ...
    // Two consecutive LLM failures followed by a success.
    let provider = Arc::new(
        FakeProvider::new(vec!["ok".to_string()])
    ...
    assert_eq!(
        recovery_events.len(),
        2,
        "expected 2 Recovery events (one per LLM error), got {}: {:#?}",
        recovery_events.len(),
        recovery_events
    );
...
/// `run_recovery_cascade` and emit a `Recovery` event whose
...
///
/// Assertions:
///   - At least one `Recovery` event with
...
async fn tool_execution_error_runs_recovery_cascade() {
    ...
    // Verify: a `Recovery` event fires with the actual tool name
    // (`bash`) — not the LLM-sample sentinel. The cascade applies L5
    // (reset) on the first failure because the consecutive-failure
    // counter starts at 0 and the L3 fallback requires 2+ failures;
    // L4 is skipped (low context ratio).
    let recovery = events
        .iter()
        .find_map(|e| {
            if let AgentEvent::System(SystemEvent::Recovery {
        ...
        .unwrap_or_else(|| {
            panic!(
                "expected a Recovery event for the tool error path, got: {:#?}",
                events
                    .iter()
                    .map(|e| match e {
                        ...
                        AgentEvent::Model(ContentPart::ToolUse(tu)) =>
                            format!("ToolCallStarted({})", tu.name),
                        other => format!("{:?}", other),
                    })
                    .collect::<Vec<_>>()
            )
...
/// wired into the tool execution error path. See the detailed
/// discussion in the original task spec.
#[tokio::test]
async fn tool_execution_cascade_emits_recovery_applied_with_actual_tool_name() {
    ...
    // Sanity: at least one Recovery event for the bash error.
    ...
    assert!(
        ...
        "expected ≥1 Recovery for tool=bash, got events: {:#?}",
        events.iter().map(|e| format!("{e:?}")).collect::<Vec<_>>()
    );

    // Sanity: NO Recovery events with the LLM sentinel (the failure
    // here is tool-execution, not LLM-sampling).
    ...
    assert!(
        ...
        "tool-error path must NOT emit Recovery with tool_name=\"llm_sample\"; \
         that sentinel is reserved for LLM-sampling recovery"
    );
}

/// Task 7: 2 consecutive tool errors → L5 reset fires twice → session
/// recovers on the 3rd LLM call.
#[tokio::test]
async fn tool_execution_l5_reset_for_consecutive_failures() {
    ...
    // Verify: 2 Recovery events (one per tool failure).
    let bash_recovery: Vec<&AgentEvent> = events
        .iter()
        .filter(|e| {
            if let AgentEvent::System(SystemEvent::Recovery {
    ...
    assert_eq!(
        bash_recovery.len(),
        2,
        "expected 2 Recovery events for tool=bash, got {}: {:#?}",
        bash_recovery.len(),
        bash_recovery
            .iter()
            .map(|e| match e {
                AgentEvent::System(SystemEvent::Recovery {
                    ...
                    message,
                    ..
                }) => format!("Recovery(L{level_number}, msg={message:?})"),
                other => format!("{:?}", other),
            })
            .collect::<Vec<_>>()
    );

    // Verify: each Recovery event has level=5 and the L5 marker
    // message.
        ...
        .filter(|e| {
            if let AgentEvent::Model(ContentPart::ToolResult(tr)) = e {
                tr.is_error == Some(true)
                    && tr.tool_use_id == "call-1"
                    && tr.content.iter().any(|p| {
                        if let ContentPart::Text(t) = p {
                            t.text.contains("Conversation reset")
                        } else {
                            false
                        }
                    })
    ...
    assert!(
        ...
        "expected ≥2 ToolResult (is_error=true) for bash carrying the L5 reset marker, got {}: {:#?}",
...
async fn llm_sampling_3_consecutive_errors_l5_reset_recovers_on_4th() {
    ...
    // Verify: 3 Recovery events with level=5 and
    ...
    assert_eq!(
        llm_recovery.len(),
        3,
        "expected 3 Recovery events for tool=llm_sample, got {}: {:#?}",
        llm_recovery.len(),
        llm_recovery
            .iter()
            .map(|e| match e {
