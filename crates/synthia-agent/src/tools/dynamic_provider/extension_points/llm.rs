...
//! - **Synchronous dispatch**: handlers run on the LLM call thread. A
//!   single point that wants to do async work is currently out of scope
//!   (it would require channel-based coordination not justified today).
//!
//! # Points
//!
...
impl LlmExtensionRegistry {
    ...
    pub fn fire_response_transform(
        &self,
...
mod tests {
    ...
    #[test]
    fn chat_params_modification_reflected_in_request() {
        let reg = LlmExtensionRegistry::new();
        ...
        let result = reg.fire_chat_params(ChatParams::default());
    ...
    #[test]
    fn deterministic_transform_preserves_hash() {
        ...
        let h: MessagesHandler = Arc::new(|ev| {
            // Replace messages with a sorted, deduplicated version.
            let sorted = ev.messages.clone();
            Action::Modify(MessagesTransformInput {
        ...
        let r1 = reg.fire_messages(input.clone());
        let r2 = reg.fire_messages(input);
    ...
    #[test]
    fn non_deterministic_transform_is_detected_by_caller() {
        ...
        let Action::Modify(a) = reg.fire_messages(input.clone()) else {
            panic!("expected Modify")
        };
        let Action::Modify(b) = reg.fire_messages(input) else {
            panic!("expected Modify")
        };
    ...
    #[test]
    fn cache_breakpoint_returns_union_of_handlers() {
        let reg = LlmExtensionRegistry::new();
        ...
        let result = reg.fire_cache_breakpoint(&CacheBreakpointInput {
            session_id: "s1".into(),
        });
    ...
    fn skip_short_circuits_the_chain() {
        let reg = LlmExtensionRegistry::new();
        let skipper: ChatParamsHandler = Arc::new(|_| Action::Skip {
    ...
    fn multiple_modifiers_apply_in_registration_order() {
        ...
        let h1: SystemPromptHandler = Arc::new(|ev| {
            Action::Modify(SystemPromptTransformInput {
                session_id: ev.session_id.clone(),
                current: format!("{}\n# step1", ev.current),
            })
        });
