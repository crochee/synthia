...
> **2026-06-15 重构**：`ShellExecutor` trait 已在 `2026-06-15-p2-trait-cleanup` change 中移除（0 trait bound、0 dyn dispatch、唯一实现为 `LocalShellExecutor`），相关方法（`execute`/`spawn`）现在是 `LocalShellExecutor` 的 inherent 方法。
...
└────────┬────────┘
         ...
         ▼
┌─────────────────────┐
│LocalShellExecutor  │  Local process execution (inherent methods)
└─────────────────────┘
```
...
```rust
use synthia_agent::shell::{LocalShellExecutor, ShellCommand};
