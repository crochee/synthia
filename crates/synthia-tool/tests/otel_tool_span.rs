...
/// `run_with_context` call under test).
...
impl Tool for EchoTool {
    ...
    async fn call(&self, _input: synthia_tool::ToolInput) -> ToolOutput {
        ToolOutput::text("ok")
    }
}
...
async fn test_tool_execute_span_has_tool_name() {
    ...
    registry.register(ToolEntry::new(Arc::new(EchoTool)));

    let (spans, _guard) = install_capture_layer();
    let results = registry
        .run_with_context(vec![make_tool_use("echo")], make_context())
        .await
        .unwrap();
...
async fn test_tool_execute_span_timeout_records_timeout_error() {
    let registry = ToolRegistry::new();
    registry.register(ToolEntry::new(Arc::new(TimeoutTool)));

    let (spans, _guard) = install_capture_layer();
    let results = registry
        .run_with_context(vec![make_tool_use("slow")], make_context())
        .await
        .unwrap();
    assert_eq!(results.len(), 1);
...
async fn test_tool_execute_span_error_records_exception() {
    let registry = ToolRegistry::new();
    registry.register(ToolEntry::new(Arc::new(FailTool)));

    let (spans, _guard) = install_capture_layer();
    let results = registry
        .run_with_context(vec![make_tool_use("fail")], make_context())
        .await
        .unwrap();
    assert_eq!(results.len(), 1);
    assert!(results[0].is_error.unwrap_or(false));
