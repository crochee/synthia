...
use crate::types::{ComplexityLevel, ConversationMetrics};
...
impl ConversationAnalyzer {
    ...
    pub fn analyze(&self, conversation: &[Message]) -> ConversationMetrics {
    ...
    fn estimate_tokens(conversation: &[Message]) -> usize {
        conversation
            .iter()
            .map(|m| m.content.extract_text().map(|t| t.len() / 4).unwrap_or(0))
            .sum()
    }
        ...
        for msg in conversation.iter().rev() {
            match msg.role {
                synthia_provider::Role::Assistant => {
                    if let Some(text) = msg.content.extract_text()
                ...
                synthia_provider::Role::User
                | synthia_provider::Role::System
                | synthia_provider::Role::Tool => break,
            }
        }
    ...
    fn analyze_complexity(&self, conversation: &[Message]) -> ComplexityLevel {
        ...
        let text = last_msg.content.extract_text().unwrap_or_default();
        let words: Vec<&str> =
...
mod tests {
    ...
    fn assistant_tool_call(text: &str) -> Message {
        Message {
            role: Role::Assistant,
            content: Content::Single(ContentPart::Text(TextContent {
                text: text.to_string(),
