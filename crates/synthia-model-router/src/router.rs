...
use crate::{
    ...
    types::{ModelConfig, ModelRouter, Result, RoutingResult, RoutingStrategy},
};
...
impl ModelRouter for ModelFallbackRouter {
    async fn route(&self, conversation: &[Message]) -> Result<RoutingResult> {
        ...
        Err(anyhow::anyhow!("All model routers failed"))
    }
...
impl DefaultModelRouter {
    ...
    pub fn with_simple_strategy(models: Vec<ModelConfig>) -> Self {
        Self::new(models, Box::new(crate::strategy::SimpleStrategy::default()))
    }
}
...
impl ModelRouter for DefaultModelRouter {
    async fn route(&self, conversation: &[Message]) -> Result<RoutingResult> {
        let mut decision = crate::types::RoutingDecision::default();
...
mod tests {
    ...
    use async_trait::async_trait;
    use synthia_provider::{Message, OpenAICompatibleProvider};

    use super::*;
    use crate::types::{ModelConfig, RoutingResult};
    ...
    impl ModelRouter for MockRouter {
        async fn route(
            ...
            *count += 1;
            ...
            if self.should_fail {
                Err(anyhow::anyhow!("Mock router failed"))
            } else {
                Ok(RoutingResult {
                    provider: Arc::new(OpenAICompatibleProvider::new(
                    ...
                    config: self
                        ...
                        .first()
                        .cloned()
                        .unwrap_or_else(|| ModelConfig::openai("default")),
                    decision: crate::types::RoutingDecision::default(),
                })
            }
        }
    ...
    fn test_fallback_router_empty_routers_returns_error() {
        ...
        let result = rt.block_on(router.route(&[make_test_message()]));
    ...
    fn test_fallback_router_calls_routers_in_sequence() {
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let _ = rt.block_on(router.route(&[make_test_message()]));
