...
use crate::{Error, Result, ScheduledJob, job::Job, trigger::Trigger};
...
impl TimeWheel {
    ...
    pub fn schedule(
        ...
        Err(Error::Internal(
            "Use schedule_async in async context".to_string(),
        ))
    }
    ...
    pub async fn schedule_async(
        ...
        let delay =
            self.calculate_delay(trigger.as_ref()).ok_or_else(|| {
                Error::Internal("Failed to calculate delay".to_string())
            })?;
    ...
    pub async fn run(
        ...
        let mut add_task_rx = add_task_rx.ok_or(Error::Internal(
            "Add task receiver already taken".to_string(),
        ))?;
        let mut remove_task_rx = remove_task_rx.ok_or(Error::Internal(
            "Remove task receiver already taken".to_string(),
        ))?;
