...
/**
 * Layer 1 — Task detail page UI tests.
 *
 * Regression coverage for the `/tasks/:id` route. Previously
 * this route existed in code (and was navigable from the
 * TasksPage card list) but had no E2E coverage at all, so any
 * regression on `TaskDetailPage` went unnoticed.
 */
test.describe('Task detail page', () => {
  test('renders not-found state for an unknown task id', async ({ page }) => {
    ...
    await expect(page.getByTestId('task-detail-back')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('h1', { hasText: 'Task' })).toBeVisible();
  });

  test('back link returns to the tasks list', async ({ page }) => {
    await page.goto('/tasks/any-id');
  ...
  test('renders detail fields for a task returned by the list endpoint', async ({ page }) => {
    // Pick the first task id from the listing page so the detail
    // route has data to render. Skip if the workspace has none
    // (the test is workspace-state dependent).
    const tasks = new TasksPage(page);
    await tasks.goto();

    // The TasksPage renders each task card with a "View" link or
    // similar affordance. We probe for the common pattern: a
    // link to /tasks/<id>. If none exist, skip.
    const firstTaskLink = page
      .locator('a[href^="/tasks/"]')
      .filter({ hasNot: page.getByTestId('task-detail-back') })
      .first();
    const href = await firstTaskLink.getAttribute('href').catch(() => null);
    test.skip(!href, 'no tasks on this workspace to drill into');
    ...
    await expect(page.getByTestId('task-detail-back')).toBeVisible();

    // At least one of the rendered cards (task summary, history,
    // artifacts) must be present. We assert on the most stable
    // marker — the "Task " card title — rather than on optional
    // fields.
    // The summary card title is rendered as an `<h3>` (Radix Card
    // wrapper) and reads `Task <prefix>`. Match by heading text
    // rather than the old `nt-card__title` class which no longer
    // exists.
    await expect(page.locator('main h3', { hasText: /^Task / })).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test('pairs tool_call and tool_result artifacts by tool_use_id', async ({ page }) => {
    ...
    // pairing-by-`tool_use_id` behaviour so old tasks remain
    // readable.
    //
    // Mock the task detail endpoint so the test does not depend
    // on workspace state. Two artifacts sharing a tool_use_id
    // (one tool_call, one tool_result) must be rendered as a
    // single grouped block with both the yellow "请求" sub-block
    // and the cyan "结果" sub-block visible.
    await page.route('**/api/v1/tasks/test-tool-pair', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await page.goto('/tasks/test-tool-pair');

    // Header carries the tool name so the user can identify the
    // paired block at a glance.
    const block = page.getByTestId('task-artifact-call-1');
    await expect(block).toBeVisible({ timeout: 10000 });
    await expect(block).toContainText('工具 · shell');

    // Both sub-blocks must render — proving call + result are
    // grouped into the same tool_use_id rather than split across
    // two separate cards.
    await expect(block.locator('.nt-task__artifact-call')).toBeVisible();
    await expect(block.locator('.nt-task__artifact-result')).toBeVisible();

    // JSON in the call block must be pretty-printed rather than
    // rendered as a raw escaped string.
    const callPre = block.locator('.nt-task__artifact-call pre').first();
  ...
  test('renders error badge when tool_result has is_error', async ({ page }) => {
    // Legacy fallback path (see previous test's comment for the
    // background). The task detail endpoint returns two legacy
    // artifacts; the frontend must surface `is_error: true` as a
    // red badge on the tool block so callers can tell a failing
    // tool from a successful one.
    await page.route('**/api/v1/tasks/test-tool-error', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    await page.goto('/tasks/test-tool-error');
    const block = page.getByTestId('task-artifact-call-2');
    await expect(block).toBeVisible({ timeout: 10000 });
    await expect(block.locator('.nt-task__artifact-error')).toBeVisible();
  });

  test('shows history-empty hint when history array is empty', async ({ page }) => {
    await page.route('**/api/v1/tasks/test-no-history', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    await page.goto('/tasks/test-no-history');
    await expect(page.getByTestId('task-history-empty')).toBeVisible({
      timeout: 10000,
    });
  });

  test('Continue-in-chat link routes to /chat/:contextId', async ({ page }) => {
    // Mock the task detail endpoint so the test owns the
    // context_id it asserts on.
    await page.route('**/api/v1/tasks/test-continue', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    await page.goto('/tasks/test-continue');
  ...
  test('renders user prompt and agent text from task history', async ({ page }) => {
    ...
    // `.nt-chat__segment--tool_block` with both the yellow
    // call sub-block and the green result sub-block.
    await page.route('**/api/v1/tasks/test-with-history', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    await page.goto('/tasks/test-with-history');

    // The empty-hint must NOT appear.
    await expect(page.getByTestId('task-history-empty')).toHaveCount(0);

    // The user message card must render with the
    // chat-style role border and the user's text inside.
    ...
    // The assistant message must render with the cyan
    // chat-style role border and a status pill on the
    // chat-style metadata strip.
    ...
    // single `.nt-chat__segment--tool_block` with both
    // the yellow call sub-block and the green result
    // sub-block visible — mirroring the live chat page.
    // Tool blocks start collapsed; click the toggle to
    // expose the call / result sub-blocks for inspection.
  ...
  test('flags tool_result errors when history carries is_error', async ({ page }) => {
    // A failing tool must surface the red error state on
    // the paired tool_block the same way the live chat
    // page does. The chat-style renderer paints the result
    // sub-block red when the reconstructed `tool_block`
    // carries `is_error: true` — this test pins that
    // behaviour for the task-detail History card.
    await page.route('**/api/v1/tasks/test-history-error', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    await page.goto('/tasks/test-history-error');
    ...
    // chat-style error variant so the user can tell a
    // failing tool from a successful one.
