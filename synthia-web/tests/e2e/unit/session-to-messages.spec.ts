 ...
 * "Continue chat" link path: a user who has already chatted
 * in a session clicks "continue chat" on the session detail
 * page and the chat store must NOT end up with a duplicate
 * user prompt + assistant turn.
 *
 ...
 *
 * Terminology note: the helper module lives at
 * `src/lib/session-to-messages.ts`. The wire types
 * (`SessionDetail` / `SessionTurn`) describe one transcript
 * frame in the session's `history` array — the front-end
 * consumes them directly via `reconstructMessagesFromSession`.
 */
...
import {
  ...
  reconstructMessagesFromSession,
  type ChatMessageLike,
} from '../../../src/lib/session-to-messages';
import type { SessionDetail, SessionTurn } from '../../../src/api/types';
...
const assistantMsg = (text: string, sessionId?: string): ChatMessageLike => ({
  id: 'a1',
  role: 'assistant',
  sessionId,
  segments: [{ id: 's1', type: 'text', content: text }],
...
test.describe('firstUserPromptText', () => {
  ...
  test('returns null for a user message with no text segments', () => {
    ...
    // is skipped and the merge falls through to the sessionId
...
test.describe('mergeReconstructedMessages', () => {
  ...
  test('drops reconstructed messages when the first user prompt already exists (the "继续 chat" path)', () => {
    ...
    // session, then clicks "continue chat" on the session
    // detail page. The chat store already has the user prompt
  ...
  test('drops reconstructed messages whose sessionId is already represented in the existing store (when the user prompt is different)', () => {
    // The first-user-prompt rule (rule 1) only matches when the
    // existing and reconstructed lists share the same opening
    // user message. If a user has chatted across two sessions
    // (one wire-format session and another resumed from the
    // same store, e.g. "Hi" then "now do X"), and a later
    // session reconstructs its own history, the first user
    // prompt of the reconstructed list is the SECOND session's
    // prompt ("now do X") and the rule doesn't fire. We then
    // fall through to the sessionId rule.
    const existing = [
      userMsg('Hi'),
    ...
    // represented (the rule drops by sessionId, not by exact
    // message).
    expect(merged).toEqual(existing);
  });

  test('appends reconstructed messages with a fresh sessionId when the user prompt is new', () => {
    // First prompt differs between existing and reconstructed
    // → falls through to sessionId rule. None of the existing
    // sessionIds match, so all reconstructed entries are appended.
    const existing = [userMsg('Hi'), assistantMsg('hello', 't1')];
  ...
  test('keeps user messages without a sessionId (reconstructed) as "fresh" and appends them when the first-user-prompt rule does not fire', () => {
    // reconstructed has no first user message (only
    // sessionId-keyed assistant turns) → rule 1 is skipped.
    // The reconstructed user message has no sessionId → rule 2
    // never drops it. The reconstructed assistant message has
    // a sessionId not in the existing set → kept.
    const existing = [userMsg('Hi'), assistantMsg('hello', 't1')];
...
test.describe('reconstructMessagesFromSession — parallel tool call pairing', () => {
  ...
  //
  // These tests build a synthetic `SessionDetail` carrying an
  ...
  // Helpers — build a tool_call and tool_result Model envelope
  // in the wire shape that `session-to-messages.ts` consumes.
  // Each `Model` event carries exactly one `ContentPart` whose
  // discriminator (`type`) is `tool_use` or `tool_result`.
  const toolCallTurn = (id: string, name: string, input: Record<string, unknown>): SessionTurn => ({
    type: EVENT_MODEL,
  ...
  const toolResultTurn = (
    ...
    type: EVENT_MODEL,
    data: { type: 'tool_result', tool_use_id: toolUseId, content, is_error: isError },
  });

  const buildSession = (history: ReadonlyArray<SessionTurn>): SessionDetail => ({
    id: 's1',
    status: 'completed',
    context_id: 'c1',
    history: [...history],
  ...
  test('attaches a tool_result to the matching tool_block when two are open', () => {
    const history: SessionTurn[] = [
      ...
      toolCallTurn('call-A', 'skill', { name: 'code-review' }),
      // shell call (id=call-B) — second tool fires before
      // call-A's result has streamed back
      toolCallTurn('call-B', 'shell', { command: 'git log -1' }),
      // call-A's result arrives first
      toolResultTurn('call-A', 'skill-output'),
      // then call-B's result
      toolResultTurn('call-B', 'shell-output'),
    ];
    const messages = reconstructMessagesFromSession(buildSession(history));
  ...
  test('falls back to trailing-pending pairing when the wire result_id has no matching call', () => {
    ...
    // tool_call event was lost (e.g. an out-of-band retry
    // while resuming a wire-format session). The pairing
    // reducer should still drop the result onto a still-open
    // block (the trailing one) so a single-call transcript
    // continues to render — the pairing key is best-effort,
    // and dropping the result on the floor would be worse
    // than misattributing it to the closest pending block.
    const history: SessionTurn[] = [
      toolCallTurn('call-A', 'shell', { command: 'echo hi' }),
      toolCallTurn('call-B', 'shell', { command: 'pwd' }),
      // result for an id that doesn't match either call —
      // falls back to the trailing pending block (call-B).
      toolResultTurn('call-UNKNOWN', 'orphan result'),
    ];
    const messages = reconstructMessagesFromSession(buildSession(history));
    const blocks = messages
      .find((m) => m.role === 'assistant')!
  ...
  test('marks failing tool_results on the matching block', () => {
    ...
    const history: SessionTurn[] = [
      toolCallTurn('call-A', 'shell', { command: 'rm -rf /' }),
      toolCallTurn('call-B', 'shell', { command: 'pwd' }),
      toolResultTurn('call-A', 'permission denied', true),
      toolResultTurn('call-B', '/home/crochee'),
    ];
    const messages = reconstructMessagesFromSession(buildSession(history));
    const blocks = messages
      .find((m) => m.role === 'assistant')!
