/**
 * Conversion helpers between the task-detail wire format
 * (TaskDetail / TaskMessage / TaskArtifact) and the ChatPage's
 * `Message[]` shape persisted under `synthia.messages.<sessionId>`.
 *
 * The chat UI surfaces a conversation as a flat list of `Message`
 * objects, each with a `role` and a list of typed `segments`
 * (text, thinking, tool_block, ...). The task detail endpoint
 * persists the full conversation into `task.history` as A2A v1.0
 * `Message`s: user prompts (role=user), agent text deltas, and
 * tool_call / tool_result events with `Part::data` payloads
 * shaped like the provider's `ToolUse` / `ToolResult` structs
 * (`{id, name, input}` / `{tool_use_id, content, is_error}`).
 * The wire carries no synthetic `kind` discriminator — we
 * detect the segment kind from the keys present in each
 * `Part::data` payload.
 *
 * Source priority:
 *   1. `task.history` — primary source of truth. Each A2A
 *      message maps to one chat message (or a segment inside an
 *      in-flight assistant turn).
 *   2. `task.artifacts` — legacy fallback for tasks that were
 *      completed before history persistence landed. The artifact
 *      `metadata.kind` discriminator here is the legacy
 *      `tool_call` / `tool_result` marker; the wire shape is
 *      identical to the history path so the chat UI can render
 *      both without special-casing.
 *
 * The exported `seedChatFromTask` is the entry point used by
 * TaskDetailPage and TasksPage when the user clicks "继续 chat":
 * it merges the reconstructed messages into whatever the local
 * chat store already has, preferring the existing entry (the
 * user may have chatted in this session and we'd rather not
 * duplicate).
 */
import type { TaskArtifact, TaskDetail, TaskMessage, TaskPart } from '../api/types';
import { classifyPartPayload } from '../api/a2a-stream';
...
// Internal type used only within `task-to-messages.ts`;
...
/** A2A `role` strings as serialized by the v1 API. */
const ROLE_USER = 'ROLE_USER';
const ROLE_AGENT = 'ROLE_AGENT';

/** Legacy discriminator carried on the `metadata` of pre-history
 *  `Artifact`s. New history messages use the natural-shape
 *  detection in `api/a2a-stream.ts::classifyPartPayload`. */
const LEGACY_ARTIFACT_KIND_TOOL_CALL = 'tool_call';
const LEGACY_ARTIFACT_KIND_TOOL_RESULT = 'tool_result';

/**
 * Read the plain text from a `Part`. A2A v1.0's `Part::text`
 * carries plain text; `Part::data` is structured JSON
 * (tool_use / results), so we only consult `text`. Returning
 * `''` for non-text parts lets callers fall through to
 * `Part::data` handling (or render an empty block).
 */
function partText(part: TaskPart | undefined): string {
  if (!part) return '';
 ...
 * (`historyToChatMessages`) and the task-detail renderer
 * (`TaskDetailPage`) can apply the same rule.
 *
...
export function isToolEchoText(text: string): boolean {
  ...
  if (typeof obj.id === 'string' && 'input' in obj) {
    ...
    // task detail page — log the id so we can correlate
...
/**
 * Read a `Part::data` payload as a JSON object. Returns `null`
 * if the part carries no `data` field or the payload isn't an
 * object.
 */
function partData(part: TaskPart | undefined): Record<string, unknown> | null {
  if (!part || !part.data || typeof part.data !== 'object') return null;
 ...
 * way.
 */
function appendAgentText(messages: ChatMessageLike[], text: string, taskId: string): void {
  if (!text) return;
...
/**
 * Convert one history entry into chat messages. A `ROLE_USER`
 * entry produces one user message; agent text entries append to
 * the trailing assistant turn (coalesced); agent
 * `tool_call` / `tool_result` entries drive the tool-block
 * pairing logic above.
 */
function applyHistoryEntry(entry: TaskMessage, taskId: string, out: ChatMessageLike[]): void {
  ...
  for (const part of parts) {
    ...
    const kind = data ? classifyPartPayload(data) : null;
    if (kind === 'tool_call' && data) {
      appendToolCall(out, data, taskId);
      continue;
    }
    if (kind === 'tool_result' && data) {
      appendToolResult(out, data, taskId);
      continue;
    }
    const text = partText(part);
    // Skip the LLM's text echo of a tool event. The real
    // event is already on the wire as `Part::data` and
    // will be rendered as a tool block; the text echo
    // would otherwise append the same data to the
    // assistant's text segment, creating the appearance
    // of a duplicate tool turn.
    if (text && isToolEchoText(text)) {
      continue;
    }
    if (text) appendAgentText(out, text, taskId);
  }
}

/**
 * Build the chat messages that best represent a task's
 * `history` array. Each A2A message becomes either a standalone
 * chat message (user / tool events) or a segment appended to
 * the trailing assistant turn (text deltas coalesce; tool_call
 * opens a `tool_block`; tool_result fills the most recent open
 * block or falls back to a free-standing segment).
 *
...
function historyToChatMessages(
  history: ReadonlyArray<TaskMessage>,
  taskId: string,
): ChatMessageLike[] {
  const out: ChatMessageLike[] = [];
  for (const entry of history) {
    applyHistoryEntry(entry, taskId, out);
  }
  return out;
}
 ...
 * pair. This is the legacy fallback path used when a task has
 * no persisted history — pre-`Task.history` runs only had
 * artifacts to reconstruct from. The legacy artifact
 * `metadata.kind` carries the explicit discriminator; new
 * history messages don't need it.
 */
function artifactsToAssistantMessage(
  artifacts: ReadonlyArray<TaskArtifact>,
  taskId: string,
...
function extractArtifactText(art: TaskArtifact): string {
  if (!art.parts) return '';
  return art.parts.map((p) => (typeof p.text === 'string' ? p.text : '')).join('');
}

/**
 * Build the messages that best represent the task's history for
 * the chat UI. Prefers `task.history` (the A2A-protocol-native
 * transcript — user prompt, agent text, tool calls and
 * results); falls back to `task.artifacts` when the server has
 * not yet persisted history (legacy tasks).
 */
export function reconstructMessagesFromTask(task: TaskDetail): ChatMessageLike[] {
  const out: ChatMessageLike[] = [];
  if (task.history && task.history.length > 0) {
    out.push(...historyToChatMessages(task.history, task.id));
    return out;
  }
  const assistant = artifactsToAssistantMessage(task.artifacts, task.id);
 ...
 *      this task — return the existing store unchanged.
 *      This catches the common case where the user has
 *      chatted in this session, then clicks "继续 chat" on
 *      the task detail page; seeding would otherwise add
 *      a duplicate user prompt. The check is "any user
 *      message in the existing list" (not "first one")
 *      because a user may have multiple tasks in the same
 *      session — only the matching prompt indicates the
 *      task is already in the chat.
 *   2. Otherwise, drop reconstructed messages whose
 *      `taskId` is already represented in the existing
 *      store. `taskId` is set on the assistant turn that
 *      produced a streamed `Message(agent)` event; matches
 *      mean "this task's transcript is already in the chat".
 *   3. Reconstructed messages WITHOUT a `taskId` (i.e. the
 *      user prompt reconstructed from `Task.history`) are
 *      appended — they have no `taskId`-keyed overlap
 *      risk, and rule 1 already ruled out the same
...
export function mergeReconstructedMessages(
  ...
  // (1) Chat already in sync with this task?
  ...
  // (2) Drop reconstructed messages whose taskId is already in
  // the existing store.
  const existingTaskIds = new Set(
    existing.map((m) => m.taskId).filter((id): id is string => Boolean(id)),
  );
  const fresh = reconstructed.filter((m) => !m.taskId || !existingTaskIds.has(m.taskId));
...
/**
 * Convenience wrapper for the "继续 chat" click path. Reads the
 * current chat store under `synthia.messages.<sessionId>`,
 * merges in the reconstructed messages, and writes the result
 * back. Safe to call when nothing is stored yet — the store
 * starts as an empty array and is updated in place.
 */
export function seedChatFromTask(
  sessionId: string,
  task: TaskDetail,
  ...
  const reconstructed = reconstructMessagesFromTask(task);
