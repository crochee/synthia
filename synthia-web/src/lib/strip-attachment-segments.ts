/**
 * Drop any 'attachment' segments from a message list before
 * persisting to localStorage. Attachment payloads can be large
...
export function stripAttachmentSegments<T extends WithSegments>(messages: ReadonlyArray<T>): T[] {
  return messages.map((m) => ({
    ...m,
    segments: m.segments.filter((s) => s.type !== 'attachment'),
  }));
}
