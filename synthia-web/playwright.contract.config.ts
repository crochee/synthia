...
export default defineConfig({
  ...
  // `sse-harness.test.ts` unit test imports from `vitest` and is
  // run via `npm run test:unit`; including it here makes Playwright
  // crash with "Vitest failed to access its internal state".
  testMatch: /.*\.spec\.ts$/,
  fullyParallel: false,
